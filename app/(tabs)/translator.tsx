...
export default function DragDropExercise() {
  ...
  return (
    <View className="flex-1 bg-gray-50 p-4 relative">
      {/* Header */}
      <View className="flex-row items-center" style={{ height: headerHeight }}>
        <TouchableOpacity onPress={() => router.back()} className="p-1">
          <Ionicons name="close" size={24} color="#333" />
        </TouchableOpacity>
        <View className="h-2 bg-gray-200 rounded-full overflow-hidden flex-1 ml-3">
          <View className="h-full bg-blue-500 rounded-full" style={{ width: `${progress}%` }} />
        </View>
      </View>

      {/* Title */}
      <Text className="text-2xl font-bold text-gray-800" style={{ height: titleHeight }}>
        {exerciseData.title}
      </Text>
